...
router.get("/", auth, async (req, res) => {
  try {
    ...
    console.error(err.message);
    res.status(500).send("Get posts error");
  }
});
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(err.message);
      res.status(500).send("Create comment to post error");
    }
  }
);
...
router.delete("/comment/:id/:comment_id", auth, async (req, res) => {
  try {
    ...
    if (comment.user.toString() !== req.user.id) {
      return res.status(401).json({ msg: "User not authorized" }); //401 not authorized error
    }
    ...
    console.error(err.message);
    res.status(500).send("Delete comment from post error");
  }
});
